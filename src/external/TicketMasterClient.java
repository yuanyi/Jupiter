...
public class TicketMasterClient {
	private static final String HOST = "http://app.ticketmaster.com";
	private static final String PATH = "/discovery/v2/events.json";
	private static final String DEFAULT_KEYWORD = "event";
	private static final int DEFAULT_RADIUS = 50;
	private static final String API_KEY = "xfMeVlAWTAWxG5pGn0B6RyLstQvfjiZd";
	
	public List<Item> search(double lat, double lon, String keyword) {
		if(keyword == null) {
			keyword = DEFAULT_KEYWORD;
		}
//		System.out.println("yy test search");
		try {
		...
		try {
			...
			if(responseCode != 200) {
				System.out.println("yy test no 200");
	...
	private List<Item> getItemList(JSONArray events) throws JSONException {
		...
		for(int i = 0; i < events.length(); i++) {
			...
			itemList.add(builder.setAddress(getAddress(event))
